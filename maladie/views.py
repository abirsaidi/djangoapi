from django.shortcuts import render
from rest_framework.views import APIView
from rest_framework.response import Response
from rest_framework import status
from .serialisers import MaladieSerializer
from .models import Maladies
# Create your views here.
class MaladieCreateView(APIView):
    def post(self, request):
        serializer = MaladieSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
class MaladieListView(APIView):
    def get(self, request):
        maladies = Maladies.objects.all()
        serializer = MaladieSerializer(maladies, many=True)
        return Response(serializer.data)
class MaladieDetailView(APIView):
    def get(self, request, pk):
        maladie = Maladies.objects.get(id=pk)
        serializer = MaladieSerializer(maladie)
        return Response(serializer.data)
class MaladieUpdateView(APIView):
    def put(self, request, pk):
        maladie = Maladies.objects.get(id=pk)
        serializer = MaladieSerializer(maladie, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    def patch(self, request, pk):
        maladie = Maladies.objects.get(id=pk)
        serializer = MaladieSerializer(maladie, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)    
class MaladieDeleteView(APIView):
    def delete(self, request, pk):
        maladie = Maladies.objects.get(id=pk)
        maladie.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)        
